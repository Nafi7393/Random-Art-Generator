...
with st.sidebar.form(key="Image Details"):
    ...
    if image_type == "Rectangle":
        ...
        rectangle_size = st.slider(label="Minimum and Maximum Size of Rectangle",
                                   ...
                                   value=(min_v_rec + 20, max_v_rec - 20))
    ...
    if image_type == "Line":
        clr_choice = st.radio(label="Set Line Color", options=('Random', 'Pick One'))
        ...
        with clr1:
            start_clr = all_functions.hex_to_rgb(st.color_picker(label="Line Start Color"))
        with clr2:
            end_clr = all_functions.hex_to_rgb(st.color_picker(label="Line End Color"))
        st.markdown("---")
    ...
    pattern = st.checkbox("Seamless Pattern")
    st.markdown("---")
