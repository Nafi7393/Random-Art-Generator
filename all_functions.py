...
def image_effect(back_image, front_line, effect):
    all_effects = ["lighter", "darker", "difference", "multiply", "screen", "soft_light", "hard_light", "overlay",
                   "add", "subtract", "add_modulo", "subtract_modulo", "logical_and", "logical_or", "logical_xor"]

    the_effect = f"ImageChops.{effect}(back_image, front_line)"
    return eval(the_effect)


def get_circle_cord(center, position, radius):
...
def get_rect_cord(center, position, size):
